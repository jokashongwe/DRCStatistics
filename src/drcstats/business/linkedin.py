...
def process_linkedin(dbname: str, query: str, suffix: str = "RDC") -> List[str]:
    ...
    for company_legal_name, company_country in curr.fetchall():
        ...
        if len(contacts) <= 0:
            continue
        upload_contact_parsed(cur=curr, contacts=contacts, conn=conn, country=company_country)
    curr.close()
    conn.close()
...
def search_linkedin(company_id: str, company: str, suffix="RDC"):
    with DDGS() as ddgs:
        ...
        print(f"{len(contacts)} contacts found for company: {company} in country {suffix}")
        return contacts


if __name__ == "__main__":
    query = query = f"SELECT distinct company_legal_name, company_country FROM companies WHERE company_country = 'Democratic Republic of the Congo' order by company_legal_name asc;"
